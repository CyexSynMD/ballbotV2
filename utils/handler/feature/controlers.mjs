...
export default async (m, extra) => {
	...
	for (let b of cate.filter(_=>_.startsWith('_') && !_.endsWith('.mjs') && !_.endsWith('.json'))) {
		...
		for (let u of file) if (m.message && !isblock) (await import(`./${b}/${u}?update=${conn.db.data.set[bot].dateplugin}`)).default(m, extra).catch(e=>e_(e));
	}
	for (let u of cmdd) {
		...
		if (s(u[0], m.command) >= 1) (await import(`./${u[1]}.mjs?update=${conn.db.data.set[bot].dateplugin}`)).default(m, extra).catch(e=>e_(e));
	}
};